...
Class order_model extends CI_Model
{
	...
	function save_order($data, $contents = false)
	{
		if (isset($data['id']))
		{
			...
			$this->db->set_dbprefix('');
			$this->db->where('id', $data['id']);
			$this->db->update('gc_en_orders', $data);
			$this->db->set_dbprefix('gc_fr_');
		...
		else
		{
			...
			$this->db->set_dbprefix('');
			$this->db->insert('gc_en_orders', $data);
			$this->db->set_dbprefix('gc_fr_');
			...
			$this->db->set_dbprefix('');
			$this->db->where('id', $id);
			$this->db->update('gc_en_orders', $order);
			$this->db->set_dbprefix('gc_fr_');
		...
		if($contents)
		{
			...
			$this->db->set_dbprefix('');
			$this->db->where('order_id', $id)->delete('gc_en_order_items');
			$this->db->set_dbprefix('gc_fr_');
			...
			foreach($contents as $item)
			{
				...
				$this->db->set_dbprefix('');
				$this->db->insert('gc_en_order_items', $save);
				$this->db->set_dbprefix('gc_fr_');
			}
		}
	...
	function save_item($data)
	{
		if (isset($data['id']) && $data['id'] != 0)
		{
			...
			$this->db->set_dbprefix('');
			$this->db->where('id', $data['id']);
			$this->db->update('gc_en_items', $data);
			$this->db->set_dbprefix('gc_fr_');
